<template>
  ...
  <div class="form-section">
    <form id="form" action="" method="POST">
      ...
      <div class="message_error" v-if="updateCommentError">{{ updateCommentError }}</div>
      <div class="login-form" >
        <Button @click.prevent="updateComment"
                :width_btn="20"
                :route_btn="''"
                ...
                :disabled_btn="disabled_reg=(
                    ...
                    rating === '' ||
                    btn_pending ||
                    (formData.comment === props.current_comment &&
                    formData.advantages === props.current_advantages &&
                    formData.disadvantages === props.current_disadvantages &&
                    rating === props.current_rating))">
          {{ !btn_pending ? 'Изменить отзыв' : '' }}
        </Button>
      </div>
    </form>
  </div>
...
const rating = ref(props.current_rating);
const config = useRuntimeConfig();
const formData = reactive({
  advantages: props.current_advantages ?? '',
  disadvantages: props.current_disadvantages ?? '',
  comment: props.current_comment ?? '',
})
...
const updateCommentError = ref('');

// const urlUpdateComment = computed(() => config.public.apiBaseUrl + `comments/${useUserStore().getUser().value.id}/create`);
const urlUpdateComment = computed(() => config.public.apiBaseUrl + `comments/${props.comment_id}`);

const updateComment = () => {
  if (!disabled_reg.value){
    updateCommentError.value = '';
    openButtonPending();
    commentUpdateFormRequest().then((res) => {
      closeButtonPending();
      emit('updateComment');
      navigateTo('#content_header');
    }).catch((err) => {
      closeButtonPending();
      updateCommentError.value = `Сохранить изменения не удалась. Проверьте корректность данных.`;
      console.error('Contact form could not be send', err)
    });
  }
};

const commentUpdateFormRequest = async () => {
  return await $fetch(urlUpdateComment.value , {
    headers: {
